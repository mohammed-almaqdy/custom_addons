...
class Card(models.Model):
    ...
    status =  fields.Selection( string='Status',selection=[('value1', 'active'),('value2', 'expired'), ('value3', 'not_active')], default='value1')
    ...
    def button_done(self):
        for rec in self:
            rec.write({'status': 'value1'})
    
    def button_cancel(self):
        for rec in self:
            rec.write({'status': 'value3'})
    
    def check_expired(self):
       today = fields.Date.today()
       purchase_orders = self.env['card'].search([])
       for order in purchase_orders:
           if order.status == 'value1' and order.expired_date < today:
               order.status = 'value2'
